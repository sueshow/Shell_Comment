  ...
  * 設定權限
  ...
  <table border="1" width="30%">
    ...
    <tr>
        ...
        <td> 創建目錄 </td>
        <td> mkdir (選項)(參數) </td>
        <td> -Z 或 –context：設置安全上下文，當使用SELinux時有效 <br>
             -m 或 --mode<目標屬性>：建立目錄的同時設置目錄的權限 <br>
             -p 或 --parents：若所要建立目錄的上層目錄目前尚未建立，則會一併建立上層目錄，若上層目錄已存在，可省略。要同時建立多個目錄，只要在指令後面逐一加上新目錄名稱，以空白隔開 <br>
             -v 或 –verbose：印出建立目錄的詳細資訊 <br>
             --help：顯示幫忙訊息                      </td>
        <td> mkdir -m 700 /usr/meng/test <br>
             mkdir -p-m 750 bin/os_1                  </td>
    </tr>
    <tr>
        <td> chmod </td>
        <td> 改變檔案的權限, SUID, SGID, SBIT等等的特性 </td>
        <td> mkdir (選項)(參數) </td>
        <td> -[檔案類型]rwx[檔案擁有者之權限]rwx[檔案所屬群組之權限]---[其他人權限] <br>
             r：可讀(4)，w：可寫(2)，x：可執行(1)，-：無權限 </td>
        <td> chmod u=rwx,g=rx,o=r filename </td>
    </tr>
    <tr>
        <td> cp </td>
        <td> 複製/拷貝 </td>
        <td> cp (檔名) (路徑)：把當前目錄(檔名)拷貝到當前目的(路徑) <br>
             cp (檔名) (檔名)：複製並改名，並存放在當前的目錄下  <br>
             cp dir/*       ：複製一個目錄下的所有檔案到當前工作目錄 <br>
             cp -a /tmp/dir1：複製一個目錄到當前工作目錄 <br>
             cp -a dir1 dir2：複製一個目錄     </td>
        <td> -a：儘可能將檔案狀態、許可權等資料都照原狀予以複製 <br>
             -r：若 source 中含有目錄名，則將目錄下之檔案亦皆依序拷貝至目的地 <br> 
             -f：若目的地已經有相同檔名的檔案存在，則在複製前先予以刪除再行複製 <br>
             -u：差異複製，常用在備份裡面           </td>
        <td>  </td>
    </tr>
    <tr>
        ...
        <td> 移動/剪下 <br> 
             重新命名         </td>
        <td> mv (檔名) (路徑)：把當前目錄下的(檔名)剪下到上一級目錄的子目錄\裡 <br>
             mv (檔名) (新檔名)：把(檔名)重新命名為(新檔名)  <br>
             mv (路徑或檔名) (新路徑或新檔名)：重新命名並移動至(新目錄)   </td>
        <td>  </td>
        <td>  </td>
    </tr>
    <tr>
        ...
        <td> rm (檔名)：直接刪除(檔名) <br>
             ...
             rm -i (路徑或檔名)：互動模式 <br>
             rm -d：刪除空白目錄          <br>
             rm -r：刪除有內容的目錄,將指定目錄下的所有檔案及子目錄一併處理，遞迴刪除 <br>
             rm -rf (路徑或檔名)：強制刪除目錄或檔案(目錄有內容時，也刪除) <br>
             rmdir (資料夾)：刪除資料夾(目錄)</td>
        <td> -f：強制移動，不會出現警告資訊 <br> 
             -i：互動模式，移動之前詢問你   <br>
             -u：目標存在的時候，只有source比較新才會更新 </td>
        <td>  </td>
    </tr>
  </table>
* 解壓縮/壓縮檔案
